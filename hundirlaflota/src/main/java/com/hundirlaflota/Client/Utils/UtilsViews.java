...
import com.hundirlaflota.Client.OnSceneVisible;
...
public class UtilsViews {
    ...
    public static void addView(Class<?> cls, String name, String path, String css) throws Exception {
        ...
        view.getStylesheets().add(cls.getResource(css).toExternalForm());
    ...
    public static void setView(String viewId) {
        ...
        for (Node n : list) {
            if (n.getId().equals(viewId)) {
                n.setVisible(true);
                n.setManaged(true);
                //call onSceneVisible() for the current view
                System.out.println("Calling onSceneVisible() " + viewId);
                
                if (getController(viewId) instanceof OnSceneVisible) {
                    ((OnSceneVisible)getController(viewId)).onSceneVisible();
                }
            } else {
                n.setVisible(false);
                n.setManaged(false);
